...
testfolders{1}='190717_gh146dalpha7_age13days';
testfolders{2}='190729_gh146dalpha7_ihc_age_6days';
testfolders{3}='190919_gh146dalpha7_behaviorIHC_age10days';
...
trainingonly = 0; % 0 for all data, 1 for training data only
if trainingonly
...
ypred = predict(pcmodel,transpose(mean([scoreR(2*go-1,componentR)'; scoreR(2*go,componentR)'])));
figure
plot(ypred,odorb(go),'o','Color',[0.2 0.7 0.2],'LineWidth',3)
...
figure
plot(calciumpredtrain,odorb(go),'o','Color',[0 0.7 0],'LineWidth',3)
...
figure
plot(ypred,odorbTest(go2),'o','Color',[0.0 0.8 0.0],'LineWidth',3)
...
hold on
plot(calciumpred,odorbTest(go2),'o','Color',[0.0 0.7 0.0],'LineWidth',3)
text(0.0,0.1,['r = ' num2str(rd(1,2),'%02.2f')],'FontSize',15)
text(0.0,0.1,['p = ' num2str(pd(1,2),'%02.2f')],'FontSize',15)
xlabel('predicted preference')
ylabel('measured preference')
...
sum(rpc<0)/length(rpc)
sum(rdm2dc2>0)/length(rdm2dc2)
