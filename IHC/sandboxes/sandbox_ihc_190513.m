...
finalLabels=zeros(size(clusterVols{1}));
clear clusterVolsNew clusterInfoNew
newcno=1;
for i=1:length(clusterVols)
   ...
   if length(splitInfo{i})>1
       ...
       for kkk=1:length(splitInfo{i})
           ...
           clusterVolsNew{newcno}=newv1;
           clusterInfoNew{newcno}=regionprops(newv1,'all');
           finalLabels(clusterVolsNew{newcno}(:)==1)=newcno;
           newcno=newcno+1;
       end
   else
       clusterVolsNew{newcno}=clusterVols{i};
       clusterInfoNew{newcno}=clusterInfo{i};
       finalLabels(clusterVolsNew{newcno}(:)==1)=newcno;
       newcno=newcno+1;
   end
...
rng('default')
mycmap=hsv(length(clusterVolsNew));
mycmap=mycmap(randperm(length(mycmap)),:);

randv=randperm(length(clusterVolsNew));

for i=randv
    p2=patch(isosurface(clusterVolsNew{i}),'FaceColor',mycmap(i,:),'EdgeColor','none','FaceAlpha',0.8);
    isonormals(clusterVolsNew{i},p2)
    %text(clusterInfo2{i}.Centroid(1),clusterInfo2{i}.Centroid(2),clusterInfo2{i}.Centroid(3),num2str(i),'Color',[0 0 0],'FontSize',25)
    drawnow
end
...
%while nomoresplits
    ...
    splitthistime=0;
    for ws=1:max(newlabels(:))
        temp=score(newlabels(:)==ws,:);
        if sum(any(isfinite(temp)'))>2
            ...
            if twokmeansum(ws)<onekmeansum(ws)
                splitthistime=1;
                pixels=find(newlabels==ws);
                newlabels(pixels(ktemp2==1))=ws;
                newlabels(pixels(ktemp2==2))=max(newlabels(:))+1;
...
lbls=regionprops(newlabels,'all');
todelete=[];
clusterVols2=cell(1,length(lbls));
clusterInfo2=cell(1,length(lbls));
bb=zeros(length(lbls),7);
for j=1:length(lbls)
    if lbls(j).Area>areathresh && lbls(j).Area<areathreshhigh
        clusterVols2{j}=(newlabels==j);
        clusterInfo2{j}=lbls(j);
        
        bb(j,:)=[clusterInfo2{j}.BoundingBox clusterInfo2{j}.FilledArea];
    else
        newlabels(newlabels==j)=0;
        todelete=[todelete j];
    end
    if mod(j,100)==0
        disp(['post-processed ' num2str(j)])
...
%% use 3d convolution of a sphere to split touching glomeruli
splitInfo=cell(1,length(clusterVols2));
finalLabels=zeros(size(clusterVols2{1}));
clear clusterVolsNew clusterInfoNew
newcno=1;
for i=1:length(clusterVols2)
    ballr=round(nthroot(clusterInfo2{i}.Area,3)/2);
   tempBall= strel('sphere',ballr);
   tempconv=convn(clusterVols2{i},tempBall.Neighborhood,'same');
   
   t=tempconv>prctile(tempconv(tempconv~=0),95);
   r1=regionprops(t);
   splitInfo{i}=r1;
   
   if length(splitInfo{i})>1
       p=clusterInfo2{i}.PixelList;
       pidx=clusterInfo2{i}.PixelIdxList;
       clear tempd
       for kkk=1:length(splitInfo{i})
           tempc(kkk,:)=splitInfo{i}(kkk).Centroid;
           tempd(kkk,:)=sqrt(sum((p-tempc(kkk,:)).^2,2));
       ...
       for kkk=1:length(splitInfo{i})
           newv1=logical(zeros(size(clusterVols2{i})));
           newv1(pidx(ind==kkk))=1;
           clusterVolsNew{newcno}=newv1;
           clusterInfoNew{newcno}=regionprops(newv1,'all');
           finalLabels(clusterVolsNew{newcno}(:)==1)=newcno;
       ...
       clusterVolsNew{newcno}=clusterVols2{i};
       clusterInfoNew{newcno}=clusterInfo2{i};
       finalLabels(clusterVolsNew{newcno}(:)==1)=newcno;
       newcno=newcno+1;
   end
   
   
%    % find principal axis along which to search for peaks
%    
%   % r=regionprops(tempconv>0,'all');
%    %[coe sco]=pca(r.PixelList);
%    
%    % collapse x and y 
%    tc=squeeze(nansum(nansum(tempconv)));
%    tc=tc/sum(tc);
%    
%    % find peaks in z. if more than 1, split at that z location
%    
%    [pks locs]=findpeaks(tc);
%    
%    plot(tc)
%    hold on
%    plot(locs,pks,'ro')
