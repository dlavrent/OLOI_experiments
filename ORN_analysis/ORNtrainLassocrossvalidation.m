...
for jjj=1:iters
    ...
    B = lasso(meanTrainData',flyTrainBehavior,'Alpha',lasso_alpha);
    mypredictor=B(:,1);
...
rsq=(mycorr.^2).*sign(mycorr);
rsqshuffled=(mycorrshuffled.^2).*sign(mycorrshuffled);
...
for jjj=1:iters
    ...
    for i=1:length(flyindicestrain)
       meanTrainData(:,i)=mean(traindata(:,datapts{i}),2);
       flyTrainBehavior(i)=mean(behaviorData(flyindicestrain{i}));
    end
    ...
    myprediction=meanTrainData'*mypredictor;
    [r p]=corrcoef(myprediction,flyTrainBehavior);
    
    [rshuffled pshuffled]=corrcoef(myprediction,flyTrainBehavior(randperm(length(flyTrainBehavior))));
