...
for i=1:length(manualLabelledFolders)
    ...
    if ~strcmp(currname(1),'.')
        ...
        behaviorOccT=[behaviorOccT occ-preocc];
        behaviorpreOccT=[behaviorpreOccT preocc];
        ...
        for j=1:length(manualClusterLabels)
            for k=1:length(publishedOR.gh146glomerulusNames)
                if strcmp(labels{i}.manual{j},publishedOR.gh146glomerulusNames{k})
                    ...
                    for oo=1:length(odortimesfortimecourse)
                        responseTempTimeCourse((((k-1)*nodors+1):k*nodors),oo)=grnResponse(:,j,odortimesfortimecourse(oo))-nanmean(grnResponse(:,j,1:3),3);
                        %responsesTensorTemp(k,oo,:)=grnResponse(oo,j,odortimesfortimecourse);
                    end
                    break
                end
        ...
        responsesGlomByOdor=[responsesGlomByOdor responseTempT(:)];
        responsesTimeCourseGlomByOdor=[responsesTimeCourseGlomByOdor responseTempTimeCourseT(:)];
        glombyodorflies=[glombyodorflies (flyNumT+(cL-1)*leftRightSplitVal)];
...
% data points that correspond to each fly
flyindicesT=cell(1,flyNumT);
flyindicesLT=cell(1,flyNumT);
flyindicesRT=cell(1,flyNumT);
for i=1:flyNumT
    [temp temp2]=find(glombyodorflies==i);
    [tem3 temp4]=find(glombyodorflies==(i+leftRightSplitVal));
    flyindicesT{i}=[temp2 temp4];
    flyindicesLT{i}=temp2;
    flyindicesRT{i}=temp4;
...
responsesNoResponseRemovedTest=responsesTimeCourseGlomByOdor;

gNames=publishedOR.gh146glomerulusNames;
glomsFound=glomfound;
numFinite=sum(isfinite(responsesNoResponseRemovedTest),2);
toRemove=find(numFinite/size(responsesNoResponseRemovedTest,2)<=fracIn);
responsesNoResponseRemovedTest(toRemove,:)=[];
...
% % fill nans with mean
for i=1:size(responsesNoResponseRemovedTest,1)
    for j=1:size(responsesNoResponseRemovedTest,2)
        if isnan(responsesNoResponseRemovedTest(i,j))
            responsesNoResponseRemovedTest(i,j)=nanmean(responsesNoResponseRemovedTest(i,:));
        end
    end
end

%% Test data evaluation: decode behavior using PCs corresponding to each time point individually
pcstouse=[4];
for j = 1:tc
    ...
    behaviorprediction=(responsesNoResponseRemovedTest')*tempCoeff(:,pcstouse);
    flyTruePref=zeros(1,flyNumT);
    flyPredictedPref=zeros(1,flyNumT);
    ally=behaviorOccT';
    
    
    nactivity=zeros(flyNumT,length(pcstouse));
    for i=1:flyNumT
        flyTruePref(i)=mean(ally(flyindicesT{i}));
        flyPredictedPref(i)=mean(mean(behaviorprediction(flyindicesT{i},:)));
        nactivity(i,:)=mean(behaviorprediction(flyindicesT{i},:));
    end
    linmodel=fitlm(nactivity,flyTruePref);
    r2(j) = linmodel.Rsquared.Adjusted;
    
end

figure;
plot(1.2:1.2:25,r2,'k*-','LineWidth',3,'MarkerSize',15)
hold on
plot(6*ones(5,1),linspace(0,0.18,5),'k--')
text(6.2,0.18,'odor on','FontSize',15)
plot(10*ones(5,1),linspace(0,0.18,5),'k--')
...
box off
set(gca,'FontSize',15)
%% cross temporal decoding evaluated on test data
...
for j = 1:tc
    ...
    for k = 1:tc
        ...
        tempCoeff2=zeros(size(COEFF));
        tempCoeff2(k:tc:end,:)=COEFF(j:tc:end,:);
        behaviorprediction2=(responsesNoResponseRemovedTest')*tempCoeff2(:,pcstouse);
        nactivityT=zeros(flyNumT,length(pcstouse));
        for i=1:flyNumT
            flyTruePrefT(i)=mean(ally(flyindicesT{i}));
            nactivityT(i,:)=mean(behaviorprediction2(flyindicesT{i},:));
        end
        preds = predict(linmodel,nactivityT);
        ctd(j,k)=(corr(preds,flyTruePrefT')^2)*sign(corr(preds,flyTruePrefT'));

    end
end

figure
imagesc(1.2:1.2:25,1.2:1.2:25,ctd,[-0.025 0.4])
ylabel('train time (s)')
xlabel('test time (s)')
hcb=colorbar;
title(hcb,'R^2')
