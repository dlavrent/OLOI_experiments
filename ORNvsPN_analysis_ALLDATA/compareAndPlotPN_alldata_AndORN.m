...
rng(1234)
ndim = size(gh146flyaverage, 1);
nPNflies = size(gh146flyaverage, 2);
nORNflies = size(orcoflyaverage, 2);

gh146flyaverageSHUFFLE = zeros(size(gh146flyaverage)); 
orcoflyaverageSHUFFLE = zeros(size(orcoflyaverage));
for i=1:nPNflies
    curflytemp = gh146flyaverage(:, i);
    gh146flyaverageSHUFFLE(:, i) = curflytemp(randperm(ndim));
end
for i=1:nORNflies
    curflytemp = orcoflyaverage(:, i);
    orcoflyaverageSHUFFLE(:, i) = curflytemp(randperm(ndim));
end



[coeffP scoreP latentP tsqP explainedP] = pca(gh146flyaverage');
[coeffO scoreO latentO tsqO explainedO] = pca(orcoflyaverage');

[coeffPSHUF scorePSHUF latentPSHUF tsqPSHUF explainedPSHUF] = pca(gh146flyaverageSHUFFLE');
[coeffOSHUF scoreOSHUF latentOSHUF tsqOSHUF explainedOSHUF] = pca(orcoflyaverageSHUFFLE');


plotndimstoshow = 10;
figure; %3
hold on
plot(1:plotndimstoshow, explainedP(1:plotndimstoshow),'LineWidth',2);
plot(1:plotndimstoshow, explainedPSHUF(1:plotndimstoshow),'LineWidth',2);
xlabel('PC #')
ylabel('% variance explained')
legend('PN calcium', 'shuffled PN calcium')

figure; %4
hold on
plot(1:plotndimstoshow, explainedO(1:plotndimstoshow),'LineWidth',2);
plot(1:plotndimstoshow, explainedOSHUF(1:plotndimstoshow),'LineWidth',2);
xlabel('PC #')
ylabel('% variance explained')
legend('ORN calcium', 'shuffled ORN calcium')


tmpP = find(explainedP - explainedPSHUF < 0);
lastkeptPNPC = tmpP(1)-1;

tmpO = find(explainedO - explainedOSHUF < 0);
lastkeptORNPC = tmpO(1)-1;

total_explained_var_PNs = sum(explainedP(1:lastkeptPNPC))
total_explained_var_ORNs = sum(explainedO(1:lastkeptORNPC))
...
figure; %11
plot(meanPublishedResponseorco,orc,'o','Color',ocolor,'LineWidth',3)
hold on
plot(meanPublishedResponsegh146,gh,'*','Color',pcolor,'LineWidth',3)
...
figure; %23
plot(EXPLAINED,'-','Color',pcolor,'LineWidth',3)
hold on
plot(EXPLAINEDO,'o-','Color',ocolor,'LineWidth',3)
ylabel('variance explained (%)')
xlabel('PC #')
legend('PNs','ORNs')
legend boxoff
...
figure; %24
semilogy(EXPLAINED,'-','Color',pcolor,'LineWidth',3)
hold on
semilogy(EXPLAINEDO,'o-','Color',ocolor,'LineWidth',3)
ylabel('variance explained (%)')
xlabel('PC #')
legend('PNs','ORNs')
legend boxoff
...
for i=1:8
    subplot(2,4,i)
    plot(COEFF(:,i),'*','Color',pcolor,'LineWidth',2,'MarkerSize',8)
    hold on
...
for i=1:8
    subplot(2,4,i)
    plot(COEFFO(:,i),'*','Color',ocolor,'LineWidth',2,'MarkerSize',8)
    hold on
...
figure %37
errorbar(1,nanmean(combined(groupings==1)),nanstd(combined(groupings==1))/sqrt(flyNum),'s','LineWidth',4,'Color',pcolor,'MarkerSize',10)
hold on
errorbar(2,nanmean(combined(groupings==2)),nanstd(combined(groupings==2))/sqrt(flyNumO),'s','LineWidth',4,'Color',ocolor,'MarkerSize',10)
errorbar(3,nanmean(combined(groupings==3)),nanstd(combined(groupings==3))/sqrt(flyNum),'s','LineWidth',4,'Color',pcolor,'MarkerSize',10)
errorbar(4,nanmean(combined(groupings==4)),nanstd(combined(groupings==4))/sqrt(flyNumO),'s','LineWidth',4,'Color',ocolor,'MarkerSize',10)
ylabel('coding space distance')
xlabels{1}='within';
xlabels{2}='across';
xlim([0 5])
