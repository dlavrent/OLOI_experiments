...
%                         end
    ...
    % make odor prediction from neural activity
    odorpred = zeros(5,13*size(responsesNoResponseRemoved,2));
    odory = zeros(1,13*size(responsesNoResponseRemoved,2));
    jj=0;
    for op = 1:13
        for zz = 1:size(responsesNoResponseRemoved,2)
            jj=jj+1;
            odorpred(:,jj)=responsesNoResponseRemoved(op:13:end,zz)/mean(responsesNoResponseRemoved(:,zz));
            odory(jj)=op;
        end
    end
    temp = randperm(length(odory));
    sp = round(trsplit*length(temp));
    tr = temp(1:sp); te = temp((sp+1):end);
    
    disp(['fitting logistic regression. PN time point = ' num2str(odortimes)])
    [B]=mnrfit(transpose(odorpred(:,tr)),categorical((transpose(odory(tr)))));
    pihat = mnrval(B,transpose(odorpred(:,te)));
    pihatcg{odortimes}=pihat;
    [val in] = max(pihat,[],2);
    classact = sum(transpose(in) == odory(te))/length(odory(te));
    odorpredacc(odortimes)=classact;
    
    pihat = mnrval(B,transpose(odorpred(:,te(randperm(length(te))))));
    [val in] = max(pihat,[],2);
    classact = sum(transpose(in) == odory(te))/length(odory(te));
    odorpredacc_shuff(odortimes)=classact;
    
    confusionMat = zeros(13,13);
    for iii = 1:length(odory(te))
        confusionMat(odory(te(iii)),in(iii))=confusionMat(odory(te(iii)),in(iii))+1;
    end
    conmat(:,:,odortimes)=confusionMat;
    
    [B]=mnrfit(transpose(odorpred(:,te)),categorical((transpose(odory(te)))));
    pihat = mnrval(B,transpose(odorpred(:,tr)));
    [val in] = max(pihat,[],2);
    classact = sum(transpose(in) == odory(tr))/length(odory(tr));
    odorpredacc(odortimes)=(odorpredacc(odortimes)+classact)/2
    
    pihat = mnrval(B,transpose(odorpred(:,tr(randperm(length(tr))))));
    [val in] = max(pihat,[],2);
    classact = sum(transpose(in) == odory(tr))/length(odory(tr));
    odorpredacc_shuff(odortimes)=(odorpredacc_shuff(odortimes)+classact)/2
    
    for iii = 1:length(odory(tr))
        confusionMat(odory(tr(iii)),in(iii))=confusionMat(odory(tr(iii)),in(iii))+1;
    end
    conmat(:,:,odortimes)=confusionMat;
end

gh146acc = odorpredacc;
gh146acc_shuff = odorpredacc_shuff;
...
%                         end
    ...
    disp(['fitting logistic regression. ORN time point = ' num2str(odortimes)])
    [B]=mnrfit(transpose(odorpred(:,tr)),categorical((transpose(odory(tr)))));
    pihat = mnrval(B,transpose(odorpred(:,te)));
    pihatco{odortimes}=pihat;
    [val in] = max(pihat,[],2);
    classact = sum(transpose(in) == odory(te))/length(odory(te));
    odorpredacc(odortimes)=classact;
    ...
    conmato(:,:,odortimes)=confusionMat;
...
plot(1.2*(1:20),gh146acc,'-','Color',pcolor,'LineWidth',3,'MarkerSize',15)
plot(1.2*(1:20),orcoacc_shuff,'--','Color',ocolor,'LineWidth',3,'MarkerSize',15)
plot(1.2*(1:20),gh146acc_shuff,'--','Color',pcolor,'LineWidth',3,'MarkerSize',15)
axis([0 25 0 0.7])
set(gca,'FontSize',15)
box on


%% use all time points to decode odor identity
...
%                         end
    ...
    gall=[gall; responsesNoResponseRemoved];
    
end
%
% make odor prediction from neural activity
odorpred = zeros(5*20,13*size(gall,2));
odory = zeros(1,13*size(gall,2));
jj=0;
for op = 1:13
    for zz = 1:size(gall,2)
        jj=jj+1;
        odorpred(:,jj)=gall(op:13:end,zz)/mean(gall(:,zz));
        odory(jj)=op;
    end
end
...
[coe sco as asd exp]=pca(odorpred');
inds = find(cumsum(exp)>95);
toppc=inds(1);

disp(['fitting logistic regression gh146.'])
[B]=mnrfit(sco(tr,1:toppc),categorical((transpose(odory(tr)))));
pihat = mnrval(B,sco(te,1:toppc));
pihatco{odortimes}=pihat;
[val in] = max(pihat,[],2);
classact = sum(transpose(in) == odory(te))/length(odory(te));
godorpredacc=classact;

[B]=mnrfit(sco(te,1:toppc),categorical((transpose(odory(te)))));
pihat = mnrval(B,sco(tr,1:toppc));
[val in] = max(pihat,[],2);
classact = sum(transpose(in) == odory(tr))/length(odory(tr));
godorpredacc=(godorpredacc+classact)/2;
...
%                         end
    ...
    oall=[oall; responsesNoResponseRemoved];
end

% make odor prediction from neural activity
odorpred = zeros(5*20,13*size(oall,2));
odory = zeros(1,13*size(oall,2));
jj=0;
for op = 1:13
    for zz = 1:size(oall,2)
        jj=jj+1;
        odorpred(:,jj)=oall(op:13:end,zz)/mean(oall(:,zz));
        odory(jj)=op;
    end
end
...
[val in] = max(pihat,[],2);
classact = sum(transpose(in) == odory(te))/length(odory(te));
oodorpredacc=classact;

[B]=mnrfit(sco(te,1:toppc),categorical((transpose(odory(te)))));
pihat = mnrval(B,sco(tr,1:toppc));
[val in] = max(pihat,[],2);
classact = sum(transpose(in) == odory(tr))/length(odory(tr));
oodorpredacc=(oodorpredacc+classact)/2;
