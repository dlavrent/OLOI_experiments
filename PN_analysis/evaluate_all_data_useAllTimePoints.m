...
tofillMaster = logical(zeros(size(responsesNoResponseRemoved)));
...
if medianResponseOrTimeCourse
    ...
    for i=1:length(odorstoremove)
        ...
        for j=1:(size(responsesNoResponseRemoved,1)/nodors)
            ...
            if isfinite(nanmean(responsesNoResponseRemoved(((j-1)*nodors+odorstoremove(i)),89:end)))
                ...
                tofillMaster(((j-1)*nodors+odorstoremove(i)),tofill)=1;
            end
        end
    end
    ...
    for j=1:(size(responsesNoResponseRemoved,1)/nodors)
        ...
        if isfinite(nanmean(responsesNoResponseRemoved(((j-1)*nodors+11),[1:89 101:end])))
            ...
            tofillMaster(((j-1)*nodors+11),tofill)=1;
        end
    end
    ...
    load('missingOdorsToFillMaster.mat')
    tofillTimeCourse = zeros(size(responsesTimeCourseGlomByOdor));
    tofillTimeCourseMCH = zeros(size(responsesTimeCourseGlomByOdor));
    tc = length(odortimesfortimecourse);
    numGlomFeatures = nodors*tc;
    
    % first 5 odors
    for k = 1:size(responsesNoResponseRemoved,2)
        for i = 1:length(publishedOR.gh146glomerulusNames)
            for j = [2 6 9 10 12]
                if tofillMaster((i-1)*nodors+j,k)==1
                    tofillTimeCourse(((i-1)*numGlomFeatures+(j-1)*tc+1):((i-1)*numGlomFeatures+(j-1)*tc+tc),k) = 1;
                end
            end
            
        end
        filltemp = find(tofillTimeCourse(:,k));
        responsesNoResponseRemoved(filltemp,k)=nanmean(responsesNoResponseRemoved(filltemp,89:end),2);
    end
    ...
    for k = 1:size(responsesNoResponseRemoved,2)
        for i = 1:length(publishedOR.gh146glomerulusNames)
            for j = 11
                if tofillMaster((i-1)*nodors+j,k)==1
                    tofillTimeCourseMCH(((i-1)*numGlomFeatures+(j-1)*tc+1):((i-1)*numGlomFeatures+(j-1)*tc+tc),k) = 1;
                end
            end
            
        end
        filltemp = find(tofillTimeCourseMCH(:,k));
        responsesNoResponseRemoved(filltemp,k)=nanmean(responsesNoResponseRemoved(filltemp,[1:89 101:end]),2);
    end
end
...
gNames(1)=[];
responsesNoResponseRemoved(1:260,:)=[];
...
figure; %5
plot(1.2:1.2:25,r2,'-','Color',pcolor,'LineWidth',3,'MarkerSize',15)
hold on
...
% SUP FIG OCT-MCH pref prediction vs time, PN PC2
figure %7
plot(1.2*(1:20),myr2,'Color',pcolor,'LineWidth',3)
xlabel('time')
ylabel('R^2')
