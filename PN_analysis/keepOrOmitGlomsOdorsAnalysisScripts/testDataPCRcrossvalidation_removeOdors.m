...
for odorKeep=1:nodors
    disp(['keeping only odor ' num2str(odorKeep)])
    ...
    % remove all but one odor
    for removeOdors=1:nodors
        if removeOdors~=odorKeep
            responsesNoResponseRemoved(removeOdors:nodors:end,:)=0;
        end
    end
    ...
    for jjj=1:iters
        ...
        if jjj==1
            rawPC{odorKeep}=zeros(length(gNames),iters,highestPCtouse);
        end

        for pcstouse=1:highestPCtouse
            currpc=COEFF(:,pcstouse);
            
            rawPC{odorKeep}(:,jjj,pcstouse)=currpc(odorKeep:nodors:end);
            ...
            testR(pcstouse,jjj,odorKeep)=myr(1,2);
            testRshuffled(pcstouse,jjj,odorKeep)=myrshuffled(1,2);
...
for odorKeep=1:nodors
    disp(['omitting odor ' num2str(odorKeep)])
    if medianResponseOrTimeCourse
        responsesNoResponseRemoved=responsesGlomByOdor;
    else
        responsesNoResponseRemoved=responsesTimeCourseGlomByOdor;
    ...
    for jjj=1:iters
        ...
        if jjj==1
            rawPC{odorKeep}=zeros(size(COEFF,1),iters,highestPCtouse);
        end

        for pcstouse=1:highestPCtouse
            currpc=COEFF(:,pcstouse);
            
            rawPC{odorKeep}(:,jjj,pcstouse)=currpc;
