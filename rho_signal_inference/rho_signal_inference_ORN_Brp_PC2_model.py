...
############## CALCIUM - BEHAVIOR
df_ORN_Brp = pd.read_csv('data/ORN_Brp_scores.csv')


ORN_PC2_scores = df_ORN_Brp['ORN Brp-Short PC2'].values
ORN_OCT_MCH_behav_scores = df_ORN_Brp['OCT-MCH preference'].values 



ORNPC2_OCTMCH_pred = ORN_PC2_scores #  REFREF 0.03 - 0.39 - 0.86
ORNPC2_OCTMCH_meas = ORN_OCT_MCH_behav_scores

n_cb = len(ORNPC2_OCTMCH_pred)

# get R^2
r_cb = get_r(ORNPC2_OCTMCH_pred, ORNPC2_OCTMCH_meas)
R2_cb = r_cb**2
...
for i in range(nboot):
    ...
    x_cb_bootstrap = ORNPC2_OCTMCH_pred[bootstrap_indices]
    y_cb_bootstrap = ORNPC2_OCTMCH_meas[bootstrap_indices]
...
# original data
axs[0].set_title('OCT-MCH preference predicted by ORN Brp-Short density PC2'+'\n'+\
                 r'$n=$ ' + f'{n_cb}, ' + r'$R^2=$' + '{:.2f}'.format(R2_cb))
axs[0].scatter(ORNPC2_OCTMCH_pred, ORNPC2_OCTMCH_meas, color=d_color['ORN'])
...
ORNdata = pd.read_csv('data/orcoflyaverage.txt', header=None).values

nORN = ORNdata.shape[1]
ndim = 65

ORNpca = PCA(); ORNpca.fit(ORNdata.T)

ORNshuffled = np.zeros(ORNdata.shape)
for j in range(nORN):
    ORNshuffled[:, j] = ORNdata[np.random.permutation(ndim), j]
     
    
ORNshuffpca = PCA()
ORNshuffpca.fit(ORNshuffled.T)



diffratORN = ORNpca.explained_variance_ratio_ - ORNshuffpca.explained_variance_ratio_
ORNcrossi = np.where(diffratORN < 0)[0][0]
R2_cc = np.sum(ORNpca.explained_variance_ratio_[:ORNcrossi])


def perform_ORN_boot():
    # perform bootstrapping
    bootstrapORNis = np.random.choice(np.arange(nORN), nORN, replace=True)    
    bootstrapORNdata = ORNdata[:,bootstrapORNis]

    # shuffle the bootstrapped ORN data
    bootstrapORNshuffled = np.zeros(bootstrapORNdata.shape)
    for j in range(nORN):
        bootstrapORNshuffled[:, j] = bootstrapORNdata[np.random.permutation(ndim), j]
        
    # perform PCA
    ORNbootpca = PCA()
    ORNbootpca.fit(bootstrapORNdata.T)
    ORNbootshuffpca = PCA()
    ORNbootshuffpca.fit(bootstrapORNshuffled.T)

    # find summed variance for PCs where var explained > that of shuffled
    diffratORN = ORNbootpca.explained_variance_ratio_ - ORNbootshuffpca.explained_variance_ratio_
    ORNcrossi = np.where(diffratORN < 0)[0][0]
    ORN_explained_var = np.sum(ORNbootpca.explained_variance_ratio_[:ORNcrossi])
    
    return ORN_explained_var


nboot = 10000
all_ORN_explained_vars = np.zeros(nboot)

for i in range(nboot):
    all_ORN_explained_vars[i] = perform_ORN_boot()
...
# original data
axs[0].set_title('variance explained by ORN calcium\ncompared to shuffled control'+'\n'+\
                 r'$n=$ ' + f'{nORN}, ' + r'$R^2=$' + '{:.2f}'.format(R2_cc))
axs[0].plot(1+np.arange(ndimplot), ORNpca.explained_variance_ratio_[:ndimplot], label='ORN calcium')
axs[0].plot(1+np.arange(ndimplot), ORNshuffpca.explained_variance_ratio_[:ndimplot], label='shuffled ORN calcium')
axs[0].axvline(ORNcrossi, c='k', label='cutoff PC')
axs[0].legend()
...
# result from bootstrapping
axs[1].set_title(f'{nboot} bootstrap replicates')
axs[1].hist(all_ORN_explained_vars, bins=50, color='0.6',)
...
N_exps = 10000
my_rhos = np.linspace(0, 1, 101)#101)
...
for ii in range(N_exps):
    ...
    for rr in range(N_rhos):
        ...
        r_predictor = perform_ORN_boot()**(1/4)
